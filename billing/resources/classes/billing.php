...
class billing {
	...
	public function create_subscription($domain_uuid, $plan_uuid, $options = []) {
		...
		$p = new permissions;
		$p->add('v_billing_subscriptions_add', 'temp');
		$database = new database;
		$database->app_name = 'billing';
		$database->app_uuid = 'b2c3d4e5-f6a7-8901-bcde-f12345678901';
		$database->save($array);
		unset($array);
		$p->delete('v_billing_subscriptions_add', 'temp');
	...
	public function suspend_domain($domain_uuid) {
		...
		$p = new permissions;
		$p->add('v_domains_edit', 'temp');
		$database = new database;
		$database->app_name = 'billing';
		$database->app_uuid = 'b2c3d4e5-f6a7-8901-bcde-f12345678901';
		$database->save($array);
		unset($array);
		$p->delete('v_domains_edit', 'temp');
	...
	public function generate_invoice($subscription_uuid) {
		...
		$p = new permissions;
		$p->add('v_billing_invoices_add', 'temp');
		$database = new database;
		$database->app_name = 'billing';
		$database->app_uuid = 'b2c3d4e5-f6a7-8901-bcde-f12345678901';
		$database->save($array);
		unset($array);
		$p->delete('v_billing_invoices_add', 'temp');
	...
	public function process_payment($invoice_uuid, $gateway_name, $payment_data = []) {
		...
		if ($result) {
			...
			$p = new permissions;
			$p->add('v_billing_payments_add', 'temp');
			$database = new database;
			$database->app_name = 'billing';
			$database->app_uuid = 'b2c3d4e5-f6a7-8901-bcde-f12345678901';
			$database->save($array);
			unset($array);
			$p->delete('v_billing_payments_add', 'temp');
	...
	public function complete_payment($payment_uuid, $transaction_id, $gateway_response = []) {
		...
		$p = new permissions;
		$p->add('v_billing_payments_edit', 'temp');
		$database = new database;
		$database->app_name = 'billing';
		$database->app_uuid = 'b2c3d4e5-f6a7-8901-bcde-f12345678901';
		$database->save($array);
		unset($array);
		$p->delete('v_billing_payments_edit', 'temp');
		...
		$p = new permissions;
		$p->add('v_billing_invoices_edit', 'temp');
		$database = new database;
		$database->app_name = 'billing';
		$database->app_uuid = 'b2c3d4e5-f6a7-8901-bcde-f12345678901';
		$database->save($array2);
		unset($array2);
		$p->delete('v_billing_invoices_edit', 'temp');
		...
		$p = new permissions;
		$p->add('v_billing_credits_add', 'temp');
		$database = new database;
		$database->app_name = 'billing';
		$database->app_uuid = 'b2c3d4e5-f6a7-8901-bcde-f12345678901';
		$database->save($array3);
		unset($array3);
		$p->delete('v_billing_credits_add', 'temp');
	...
	public function extend_subscription($subscription_uuid) {
		...
		$p = new permissions;
		$p->add('v_billing_subscriptions_edit', 'temp');
		$database = new database;
		$database->app_name = 'billing';
		$database->app_uuid = 'b2c3d4e5-f6a7-8901-bcde-f12345678901';
		$database->save($array);
		unset($array);
		$p->delete('v_billing_subscriptions_edit', 'temp');
